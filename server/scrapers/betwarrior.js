...
async function fetchAll(urls) {
  ...
  // Si TODAS fallaron, loguear para diagnóstico (antes era silent)
  if (failed === urls.length && urls.length > 0) {
    log(`[betwarrior] fetchAll: ${failed}/${urls.length} fallaron · last: ${lastErr?.message?.slice(0, 100) || 'unknown'}`);
  }
  return out;
}
...
async function enrichWithBetOffers(events, allPayload) {
  ...
  // Log si más de la mitad de batches fallaron (señal de problema sostenido)
  if (batchFails > 0 && batchFails >= batches.length / 2) {
    log(`[betwarrior] enrich batches: ${batchFails}/${batches.length} fallaron · last: ${lastBatchErr?.message?.slice(0, 80) || 'unknown'}`);
  }
