...
(function () {
  ...
  async function render(panel) {
      ...
      return {
        // Cierre exitoso — marca todo done, llena la barra, oculta.
          ...
          setTimeout(() => {
            ...
            if (btn) {
              btn.disabled = false;
              btn.querySelector('.ag-gen-label').textContent = 'Generar combinadas';
              btn.dataset.busy = '0';
    ...
    panel.querySelector('#agGenerate').addEventListener('click', async () => {
      // Anti double-click + cooldown 3s post-generación: cada llamada quema
      // ~10 calls a la cascada IA, no queremos que un click rabioso multiplique x5.
      const _gBtn = panel.querySelector('#agGenerate');
      if (_gBtn?.dataset.busy === '1') return;
      const _gLast = Number(_gBtn?.dataset.lastRun || 0);
      if (Date.now() - _gLast < 3000) {
