...
(function (global) {
  ...
  async function getCuratedCombos(opts = {}) {
    ...
    // Filtros avanzados (FASE 4.1) — el backend los respeta al armar el pool
    if (opts.oddMin != null && opts.oddMin > 1) q.set('oddMin', String(opts.oddMin));
    if (opts.oddMax != null && opts.oddMax > 1) q.set('oddMax', String(opts.oddMax));
    if (opts.timeWindowH != null && opts.timeWindowH > 0) q.set('timeWindowH', String(opts.timeWindowH));
    if (Array.isArray(opts.marketsAllow) && opts.marketsAllow.length) q.set('marketsAllow', opts.marketsAllow.join(','));
  ...
  async function generate(opts) {
    const ctrl = new AbortController();
    // 60s: con arquitectura dual (Groq 70B bulk para analyzeMatch + OpenAI
    // gpt-5-mini reasoning='minimal' para curator) el endpoint completa en
    // 10-30s. 60s da margen 2x. Antes era 180s pero hacía que el user viera
    // el loader rotando "Verificando cuotas en cada casa…" hasta 3min si
    // algún provider colgaba. 60s aborta a tiempo y muestra error claro.
