...
async function scrape() {
  ...
  try {
    ...
    // Timeouts generosos: el XML es 1.5MB y desde Render (US) la conexión
    // a deportespba.bplay.bet.ar (AR) puede ser lenta (3-25s observados).
    // bodyTimeout y headersTimeout custom (sin cap de 15s) para downloads grandes.
    let xml = await breaker.exec(() => withRetry(
      () => httpGet(FEED_URL, {
        ...
        bodyTimeout: 30000  // permite descarga lenta de 1.5MB
      }),
    ...
    if (!xml || xml.length < 1000) {
      ...
      try {
        xml = await httpGetNative(FEED_URL, { headers, timeout: 40000 });
      } catch (e) {
    ...
    if (!xml || xml.length < 1000) {
      ...
      // Stale-fallback agresivo: hasta 2h (era 5min). Cuotas viejas son MUCHO
      // mejores que vacío cuando la CDN se cae temporalmente.
      if (cachedEvents.length && Date.now() - cachedAt < 2 * 60 * 60_000) {
        const ageMin = Math.round((Date.now() - cachedAt) / 60_000);
        log(`[bplay-xml] serving stale cache (${cachedEvents.length} ev · ${ageMin}min old)`);
        return cachedEvents;
      }
      return [];
    }
    ...
    // Mismo stale fallback en path catch
    if (cachedEvents.length && Date.now() - cachedAt < 2 * 60 * 60_000) {
      const ageMin = Math.round((Date.now() - cachedAt) / 60_000);
      log(`[bplay-xml] catch path · serving stale (${cachedEvents.length} ev · ${ageMin}min old)`);
      return cachedEvents;
    }
    return [];
  }
