...
function parseMatch(matchNode, sportName, regionName, competitionName) {
  ...
  // Markets ampliados 2026-05-17: ahora extraemos también córners, tarjetas,
  // marcador exacto, HT/FT, DNB y totales 1T cuando Bplay XML los ofrece.
  // El XML usa typeName específico en castellano para cada uno.
  const markets = {
    h2h: null, totals: {}, btts: null, dc: null, ah: null,
    dnb: null,                    // empate no apuesta
    'ht-result': null,            // resultado primer tiempo (1X2 HT)
    'totals-ht': {},              // más/menos goles 1T
    'exact-score': null,          // marcador exacto (collection de outcomes)
    'corners-total': {},          // córners totales (con line)
    'cards-total': {}             // tarjetas totales (con line)
  };
  ...
  for (const offer of offers) {
    ...
    // ── DNB / Empate no apuesta (Draw No Bet)
    else if ((typeName === 'Empate no apuesta' || typeName === 'Draw No Bet' || /no\s*apuesta|sin\s*empate/i.test(typeName)) && !markets.dnb) {
      const homeOut = outcomes.find(o => o['@_name'] === home['@_name']);
      const awayOut = outcomes.find(o => o['@_name'] === away['@_name']);
      const homeOdd = parseOdd(homeOut?.['@_odds']);
      const awayOdd = parseOdd(awayOut?.['@_odds']);
      if (homeOdd && awayOdd) markets.dnb = { home: homeOdd, away: awayOdd };
    }
    // ── HT result (1X2 primer tiempo / "1-X-2 Descanso" en Bplay actual)
    // Bplay XML usa específicamente "1-X-2 Descanso" (verificado 2026-05-18,
    // 28 events en feed activo). El regex también captura variantes históricas.
    else if ((typeName === '1-X-2 Descanso' || typeName === '1-X-2 primer tiempo'
              || /1[\s\-]*x[\s\-]*2.*(?:1er|primer|ht|medio|descanso)/i.test(typeName)
              || /(?:primer\s*tiempo|descanso).*1-?x-?2/i.test(typeName))
             && !markets['ht-result']) {
      const homeOut = outcomes.find(o => o['@_name'] === home['@_name']);
      const awayOut = outcomes.find(o => o['@_name'] === away['@_name']);
      const drawOut = outcomes.find(o => /^empate$/i.test(o['@_name']));
      const homeOdd = parseOdd(homeOut?.['@_odds']);
      const awayOdd = parseOdd(awayOut?.['@_odds']);
      const drawOdd = parseOdd(drawOut?.['@_odds']);
      if (homeOdd && awayOdd) markets['ht-result'] = { home: homeOdd, draw: drawOdd, away: awayOdd };
    }
    // ── Totales 1T (más/menos primer tiempo)
    else if ((/m[áa]s.*menos.*(?:1er|primer|ht|medio)/i.test(typeName) || /(?:1er|primer|ht|medio).*tiempo.*m[áa]s/i.test(typeName)) && number != null) {
      const line = parseFloat(number);
      if (Number.isFinite(line)) {
        const mas = outcomes.find(o => /^M[áa]s$/i.test(o['@_name']));
        const menos = outcomes.find(o => /^Menos$/i.test(o['@_name']));
        if (!markets['totals-ht'][line]) {
          markets['totals-ht'][line] = { line, over: parseOdd(mas?.['@_odds']), under: parseOdd(menos?.['@_odds']) };
        }
      }
    }
    // ── Marcador exacto (collection: cada outcome es un score "1-0", "2-1", etc.)
    else if ((typeName === 'Marcador exacto' || typeName === 'Resultado exacto' || /resultado\s*exacto|marcador\s*exacto/i.test(typeName)) && !markets['exact-score']) {
      const scores = {};
      for (const o of outcomes) {
        const name = (o['@_name'] || '').trim();
        const odd = parseOdd(o['@_odds']);
        if (name && odd && /^\d{1,2}\s*[-:]\s*\d{1,2}$/.test(name)) {
          const normalized = name.replace(/\s/g, '').replace(':', '-');
          scores[normalized] = odd;
        }
      }
      if (Object.keys(scores).length > 0) markets['exact-score'] = scores;
    }
    // ── Córners totales (más/menos córners con línea)
    else if (/c[óo]rners?.*(?:m[áa]s|menos|totales?)|(?:m[áa]s|menos).*c[óo]rners?|tiros?\s*de\s*esquina/i.test(typeName) && number != null) {
      const line = parseFloat(number);
      if (Number.isFinite(line)) {
        const mas = outcomes.find(o => /^M[áa]s$/i.test(o['@_name']));
        const menos = outcomes.find(o => /^Menos$/i.test(o['@_name']));
        if (!markets['corners-total'][line]) {
          markets['corners-total'][line] = { line, over: parseOdd(mas?.['@_odds']), under: parseOdd(menos?.['@_odds']) };
        }
      }
    }
    // ── Tarjetas totales (más/menos tarjetas con línea)
    else if (/tarjetas?.*(?:m[áa]s|menos|totales?)|(?:m[áa]s|menos).*tarjetas?|amonestaci[óo]n/i.test(typeName) && number != null) {
      const line = parseFloat(number);
      if (Number.isFinite(line)) {
        const mas = outcomes.find(o => /^M[áa]s$/i.test(o['@_name']));
        const menos = outcomes.find(o => /^Menos$/i.test(o['@_name']));
        if (!markets['cards-total'][line]) {
          markets['cards-total'][line] = { line, over: parseOdd(mas?.['@_odds']), under: parseOdd(menos?.['@_odds']) };
        }
      }
    }
  }

  // Limpiar markets vacíos (5 core + 6 extendidos)
  ...
  if (!markets.dnb?.home && !markets.dnb?.away) markets.dnb = null;
  if (!markets['ht-result']?.home && !markets['ht-result']?.away) markets['ht-result'] = null;
  if (Object.keys(markets['totals-ht']).length === 0) delete markets['totals-ht'];
  if (Object.keys(markets['corners-total']).length === 0) delete markets['corners-total'];
  if (Object.keys(markets['cards-total']).length === 0) delete markets['cards-total'];
  if (!markets['exact-score'] || Object.keys(markets['exact-score']).length === 0) markets['exact-score'] = null;

  // Limpiar nulls (que no se serialicen)
  for (const k of Object.keys(markets)) if (markets[k] == null) delete markets[k];

  // Si no hay nada (ningún market poblado), descartar
