 ...
 *
 * INVALIDACIÓN: si el CACHED_BASE falla por DNS (ENOTFOUND) o ECONNREFUSED
 * sostenidamente, lo invalidamos para forzar re-discovery. Sin esto, una vez
 * que el dominio cacheteado se cae, TODAS las llamadas fallan hasta que el
 * breaker abra (60s). */
function isDnsError(err) {
  const msg = err?.message || '';
  return /ENOTFOUND|EAI_AGAIN|ECONNREFUSED|getaddrinfo/i.test(msg);
}
...
async function tryBases(path) {
  ...
  for (const base of bases) {
    try {
      ...
      if (isDnsError(e)) {
        dnsFails++;
        // Si la base cacheteada es la que está fallando por DNS, INVALIDAR
        // inmediatamente para que el siguiente attempt empiece desde scratch.
        if (base === CACHED_BASE) {
          log(`[codere] CACHED_BASE ${base.split('/')[2]} DNS-down — invalidando`);
          CACHED_BASE = null;
        }
      }
    }
  ...
  if (process.env.SCRAPINGBEE_KEY) {
    for (const base of bases) {
      try {
        ...
        // Si SBee tira sbee-skip o sbee-auth, no insistir con el resto de bases
        if (/sbee-(skip|auth|quota)/.test(e?.message || '')) break;
      }
    }
  }
  // Si TODAS las bases fallaron por DNS, mensaje claro
  if (dnsFails === bases.length) {
    throw new Error(`all-dns-fail: ${bases.length} bases unreachable · ${lastErr?.message?.slice(0, 80) || 'unknown'}`);
  }
  throw lastErr || new Error('all codere bases failed');
}

async function get(path) {
  // Retry agresivo en DNS-fails: el problema es típicamente transitorio
  // (resolver hiccup). Backoff 800ms × 2 = ~2.4s total para 3 attempts.
  return breaker.exec(() => withRetry(
    () => tryBases(path),
    { maxAttempts: 3, baseMs: 800, factor: 2 }
  ));
}
...
/* GameTypeIds que requestamos en /Home/GetEvents?gameTypes=...
 * Verificado contra Codere AR el 2026-05-18 vía sweep de IDs 1-5000:
 * estos son los únicos que tienen markets activos sostenidamente. */
const ALL_GAME_TYPES = [
  1,    // 1X2 / 2-way según resultados (soccer 3-way, otros 2-way)
  97,   // 2-way h2h (basket/tennis/baseball/hockey/volley/mma)
  3,    // Apuesta Sin Empate (DNB)
  2,    // Doble Oportunidad (soccer)
  18,   // Más/Menos Total Goles (soccer)
  317,  // Totals hockey
  393,  // Totals volleyball
  959,  // Totals baseball
  2083, // Totals basketball
  103,  // Totals amfootball (también marca per-team BTTS en soccer)
  159,  // Handicap genérico (basket/baseball/hockey)
  259,  // Handicap rugby
  31,   // Marcan Ambos Equipos (BTTS soccer)
  // ── EXPANDED 2026-05-18: extra markets confirmados en Codere ──
  4,    // 1X2 Handicap (Asian Handicap soccer con SpecialOddsValue)
  12,   // 1ª Parte - 1X2 → ht-result
  15,   // 2ª Parte - 1X2 → second-half-result (no en schema, ignoramos)
  27,   // 2ª Parte - Total de Goles → totals-ht (approximación)
  37    // Resultado Final (samples "1:0", "2:0") → exact-score
].join(';');
