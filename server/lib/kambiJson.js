...
/* ── Mapping de ligas ───────────────────────────────────────────────────────── */
const LEAGUE_MAP = [
  { re: /liga profesional|copa argentina|primera nacional/i, key: 'lpf' },
...
function processBetOffer(bo, sport) {
  ...
  /* Apuesta sin empate (DNB) — equivalente a h2h sin draw */
  if (cid === 1001159666) {
    const home = outcomes.find(o => o.type === 'OT_ONE');
    const away = outcomes.find(o => o.type === 'OT_TWO');
    const ph = parseOdds(home?.odds);
    const pa = parseOdds(away?.odds);
    if (!ph || !pa) return null;
    return { kind: 'h2h', value: { home: ph, draw: null, away: pa } };
  }
...
function parseKambiBetoffers(payload, sportHint = 'soccer') {
  ...
  for (const bo of payload.betOffers) {
    ...
    if (!res) continue;
    if (!byEvent.has(eid)) byEvent.set(eid, { markets: {}, bestTotal: null });
