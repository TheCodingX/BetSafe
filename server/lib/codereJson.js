...
const LEAGUE_MAP = [
  ...
  { re: /liga\s*profesional\s*(?:de\s*f[úu]tbol|argentina)|primera\s*divisi[óo]n.*argentina|copa\s*argentina|primera\s*nacional|argentina.*primera\s*divisi|\bafa\b/i, key: 'lpf' },
...
/* ── Mercados extendidos (2026-05-18, verificados contra API real) ──────────
 * Estos gtids llegan en `Results[0].GameTypeId` cuando incluimos sus IDs
 * en el parámetro `gameTypes=` del request a GetEvents. */
const GTID_HT_RESULT = 12;          // "1ª Parte - 1X2"           → ht-result
const GTID_TOTALS_HT = 27;          // "2ª Parte - Total de Goles" → totals-ht (approx)
const GTID_EXACT_SCORE = 37;        // "Resultado Final"          → exact-score
const GTID_AH_SOCCER = 4;           // "1X2 Handicap" soccer      → ah (con SpecialOddsValue)
...
function processGame(g, teams, sport) {
  ...
  if (GTID_DNB.has(gtid)) {
    const h = odd(0), a = odd(1);
    if (!h || !a) return null;
    return { kind: 'dnb', value: { home: h, away: a } };
  }

  /* ── Mercados extendidos por GameTypeId (PATH RÁPIDO, antes del path regex)
   * Verificado real-data 2026-05-18: estos gtids llegan consistentemente
   * cuando los pedimos en el parámetro gameTypes= de GetEvents. */

  // HT-result: gtid=12, "1ª Parte - 1X2", SortOrder 0/1/2 = home/draw/away
  if (gtid === GTID_HT_RESULT) {
    const h = odd(0);
    const d = results.length >= 3 ? odd(1) : null;
    const a = odd(results.length >= 3 ? 2 : 1);
    if (!h || !a) return null;
    return { kind: 'ht_result', value: { home: h, draw: d, away: a } };
  }

  // Totals 1st half: gtid=27, "2ª Parte - Total de Goles" — viene con buckets
  // ("Menos 2 goles", "2-3", "4 o más"). Solo aprovechamos si hay over/under
  // con línea SpecialOddsValue válida (puede no estar siempre).
  if (gtid === GTID_TOTALS_HT && line != null && results.length >= 2) {
    const ov = odd(0), un = odd(1);
    if (!ov || !un) return null;
    return { kind: 'totals_ht_candidate', value: { line, over: ov, under: un } };
  }

  // Exact-score: gtid=37, "Resultado Final", Results[].Name son scores ("1:0", "2:0")
  if (gtid === GTID_EXACT_SCORE) {
    const scores = {};
    for (const r of results) {
      const name = String(r.Name || '').trim();
      const o = parseOdd(r.Odd);
      const m = name.match(/^(\d{1,2})\s*[-:]\s*(\d{1,2})$/);
      if (m && o) scores[`${m[1]}-${m[2]}`] = o;
    }
    if (!Object.keys(scores).length) return null;
    return { kind: 'exact_score', value: scores };
  }

  // AH soccer: gtid=4, "1X2 Handicap" con SpecialOddsValue (línea)
  if (gtid === GTID_AH_SOCCER && line != null) {
    // 3 outcomes: home -line / draw push / away +line. Tomamos home y away.
    const ho = odd(0), ao = results.length >= 3 ? odd(2) : odd(1);
    if (!ho || !ao) return null;
    return { kind: 'ah', value: { line, home_minus: ho, away_plus: ao } };
  }

  /* Markets extendidos por NOMBRE del game (2026-05-17).
   * Codere no expone gameTypeIds estables para córners/cards/etc., entonces
   * detectamos por g.Name o results[0].Name. Si la casa no devuelve esos
   * markets para el partido, simplemente no se agregan al pool — sin error. */
  const gameName = String(g.Name || g.GameType || results[0]?.Name || '').toLowerCase();

  // Córners totales (Más/Menos córners con línea)
  if (/c[óo]rners?.*(?:m[áa]s|menos|over|under|total)|(?:m[áa]s|menos|over|under).*c[óo]rners?/.test(gameName) && line != null) {
    const ov = odd(0), un = odd(1);
    if (!ov || !un) return null;
    return { kind: 'corners_candidate', value: { line, over: ov, under: un } };
  }
  // Tarjetas totales (Más/Menos tarjetas con línea)
  if (/tarjetas?.*(?:m[áa]s|menos|over|under|total)|(?:m[áa]s|menos|over|under).*tarjetas?|amonestaci[óo]n/.test(gameName) && line != null) {
    const ov = odd(0), un = odd(1);
    if (!ov || !un) return null;
    return { kind: 'cards_candidate', value: { line, over: ov, under: un } };
  }
  // Resultado 1er tiempo (HT 1X2)
  if (/1[\s\-]*x[\s\-]*2.*(?:1er|primer|medio|ht)/.test(gameName) || /(?:primer|1er|medio)\s*tiempo.*1[\s\-]*x[\s\-]*2/.test(gameName)) {
    const h = odd(0);
    const d = results.length >= 3 ? odd(1) : null;
    const a = odd(results.length >= 3 ? 2 : 1);
    if (!h || !a) return null;
  ...
  // Más/Menos goles 1er tiempo
  if (/(?:m[áa]s|menos|over|under).*(?:goles?|puntos?).*(?:1er|primer|medio|ht)/.test(gameName) && line != null) {
    const ov = odd(0), un = odd(1);
    if (!ov || !un) return null;
    return { kind: 'totals_ht_candidate', value: { line, over: ov, under: un } };
  }
  // Marcador exacto (cada Result es un score "1-0", "2-1")
  if (/resultado\s*exacto|marcador\s*exacto/.test(gameName)) {
    const scores = {};
    for (const r of results) {
      const name = String(r.Name || '').trim();
      const o = parseOdd(r.Odd);
